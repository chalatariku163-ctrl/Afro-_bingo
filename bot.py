...
def load_data():
    ...
    try:
        ...
        cards_10 = {
            int(k): v
            for k, v in data.get("cards_10", {}).items()
        }

        cards_20 = {
            int(k): v
            for k, v in data.get("cards_20", {}).items()
        }
...
def normalize_card_data():
    ...
    for cards in [cards_10, cards_20]:
...
def get_card_data(card_type, card_number):
...
def get_card_owner(card_type, card_number):
...
async def deposit_amount(
    ...
    await query.edit_message_text(
        ...
        "1️⃣ Lakkoofsa Telebirr kana irratti kaffali.\n\n"

        "2️⃣ Erga kaffaltee booda screenshot ragaa kaffaltii qofa asitti ergi.\n\n"

        "✅ Screenshot kee erga ergitee booda request kee ni ilaalama.",
...
async def receive_deposit_photo(
    ...
    if deposit.get("status") != "waiting_screenshot":
...
async def approve_deposit(
    ...
    if deposit.get("status") != "pending_admin":
    ...
    await query.get_bot().send_message(
        ...
        text=(
            ...
            f"💰 Balance kee irratti {amount} Birr dabaleera.\n\n"

            f"💳 Balance kee: {get_balance(user_id)} Birr"
...
async def how_to_play(query):
    ...
    text = (
        ...
        "5️⃣ Card buying yeroo isaa keessatti bitadhu.\n\n"

        "6️⃣ Number ofumaan waamama.\n\n"
...
def check_bingo_api():
    ...
    with bingo_lock:
        ...
        bingo_game["winner_card_type"] = card_type
